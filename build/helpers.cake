List<NuSpecContent> GetContent(IEnumerable<string> frameworks, ProjectCollection projects, string configuration, Func<SolutionProject, bool> projectFilter = null) {
    ...
    foreach (var framework in frameworks) {
        foreach (var project in projects.SourceProjects.Where(projectFilter)) {
            ...
            var pathRoot = project.Path.GetDirectory() + "/bin/" + configuration + "/" + framework + "/";
            var match = GetFiles(pathRoot +  "publish/" + "*").Where(f => f.GetFilename().ToString().Contains("Handlebars"));
            var libFiles = match
...
public class ProjectCollection {
    ...
    public SolutionProject Get(string name) {
        return AllProjects.FirstOrDefault(p => p.Name == name);
    }
    public DirectoryPath GetPath(string name) {
        return Get(name).Path.GetDirectory();
    }
}
...
Dictionary<string, string> SetPlatformEnvironment() {
    ...
    if (DirectoryExists("/usr/lib/mono/4.5/")) {
        env.Add("FrameworkPathOverride", "/usr/lib/mono/4.5/");
    }
    return env;
}